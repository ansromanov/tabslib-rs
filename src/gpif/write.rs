...
pub fn write(doc: &Document) -> String {
    ...
    let mut index_of = |r: Rhythm| -> usize {
        if let Some(i) = table.iter().position(|x| *x == r) { i } else { table.push(r); table.len() - 1 }
    };
        ...
        for voice in &bar.voices {
            ...
            for beat in &voice.beats {
                ...
                for n in &beat.notes {
                    ...
                    if let Some(f) = n.fret { props.push_str(&format!("<Property name=\"Fret\"><Fret>{f}</Fret></Property>")); }
                    if let Some(s) = n.string { props.push_str(&format!("<Property name=\"String\"><String>{s}</String></Property>")); }
                    if let Some(m) = n.midi { props.push_str(&format!("<Property name=\"Midi\"><Number>{m}</Number></Property>")); }
                    ...
                    let vib = if n.techniques.contains(&Technique::Vibrato) { "<Vibrato>Slight</Vibrato>" } else { "" };
                    let art = n.articulation.map(|a| format!("<InstrumentArticulation>{a}</InstrumentArticulation>")).unwrap_or_default();
                    notes.push_str(&format!("<Note id=\"{nid}\">{art}{vib}<Properties>{props}</Properties></Note>"));
                    note_ids.push(nid.to_string());
                    nid += 1;
                }
                let r = index_of(beat.rhythm);
                let dyn_ = beat.dynamic.as_deref().map(|d| format!("<Dynamic>{}</Dynamic>", esc(d))).unwrap_or_default();
                let nrefs = if note_ids.is_empty() { String::new() } else { format!("<Notes>{}</Notes>", note_ids.join(" ")) };
                beats.push_str(&format!("<Beat id=\"{bid}\">{dyn_}<Rhythm ref=\"{r}\"/>{nrefs}</Beat>"));
                beat_ids.push(bid.to_string());
                bid += 1;
            }
            voices.push_str(&format!("<Voice id=\"{vid}\"><Beats>{}</Beats></Voice>", beat_ids.join(" ")));
            voice_ids.push(vid.to_string());
            vid += 1;
        }
        while voice_ids.len() < 4 { voice_ids.push("-1".into()); }
        let clef = bar.clef.as_deref().map(|c| format!("<Clef>{}</Clef>", esc(c))).unwrap_or_default();
        bars.push_str(&format!("<Bar id=\"{}\">{clef}<Voices>{}</Voices></Bar>", bar.id, voice_ids.join(" ")));
    }

    let rhythms: String = table.iter().enumerate().map(|(i, r)| {
        let dot = if r.dots > 0 { format!("<AugmentationDot count=\"{}\"/>", r.dots) } else { String::new() };
        let tup = r.tuplet.map(|(n, d)| format!("<PrimaryTuplet num=\"{n}\" den=\"{d}\"/>")).unwrap_or_default();
        format!("<Rhythm id=\"{i}\"><NoteValue>{}</NoteValue>{dot}{tup}</Rhythm>", r.value.as_gpif())
    }).collect();

    let master: String = doc.master_bars.iter().map(|m| {
        // <Section> must precede <Bars>: the element is sequenced, and Guitar
        // Pro drops a section that arrives after the bar list.
        let sec = m.section.as_deref().map(|s|
            format!("<Section><Letter>{}</Letter><Text>{}</Text></Section>", cdata(""), cdata(s))).unwrap_or_default();
        let db = if m.double_bar { "<DoubleBar/>" } else { "" };
        let refs: Vec<String> = m.bar_ids.iter().map(|i| i.to_string()).collect();
        format!("<MasterBar><Time>{}/{}</Time>{sec}{db}<Bars>{}</Bars></MasterBar>", m.time.0, m.time.1, refs.join(" "))
    }).collect();
...
<Notes>{notes}</Notes><Rhythms>{rhythms}</Rhythms></GPIF>",
        cdata(&doc.title), cdata(&doc.artist))
}
