...
fn positions(doc: &Document) -> Vec<(Option<u32>, Option<i32>, Option<i32>)> {
    doc.bars.iter().flat_map(|b| &b.voices).flat_map(|v| &v.beats)
        .flat_map(|b| &b.notes).map(|n| (n.string, n.fret, n.articulation)).collect()
}
...
fn round_trip_changes_nothing_measurable() {
    for (name, before) in fixtures::all() {
        ...
        assert_eq!(after.note_count(), before.note_count(), "{name}: note count");
        assert_eq!(after.tracks.len(), before.tracks.len(), "{name}: track count");
        assert_eq!(after.bars.len(), before.bars.len(), "{name}: bar count");
        assert_eq!(after.section_count(), before.section_count(), "{name}: sections");
        assert_eq!(durations(&after), durations(&before), "{name}: duration histogram");
        assert_eq!(techniques(&after), techniques(&before), "{name}: technique census");
        assert_eq!(positions(&after), positions(&before), "{name}: string/fret/articulation");
    }
}
...
fn every_note_value_survives_a_round_trip() {
    ...
    assert!(a.keys().any(|(_, d)| d % 3 == 0), "fixture has no triplets to test");
    assert!(a.keys().any(|(_, d)| d % 5 == 0), "fixture has no quintuplets to test");
}
...
fn meters_and_section_names_survive() {
    ...
    let after = tabslib::load(&tabslib::save(&before).expect("save")).expect("reload");
    let m = |d: &Document| d.master_bars.iter().map(|m| (m.time, m.section.clone())).collect::<Vec<_>>();
...
fn palm_mute_is_read_from_the_property_attribute() {
    ...
    assert_eq!(after.technique_count(|t| matches!(t, Technique::PalmMute)), expected);
}
